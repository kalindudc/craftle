...
public abstract class ItemStackIngredient implements IInputIngredient<ItemStack> {
    ...
    public static ItemStackIngredient deserialize(@Nullable JsonElement json) {
        ...
        if (!json.isJsonObject()) {
            throw new JsonSyntaxException("Item must be an object or an array" + " of objects");
        }
        ...
        if (jsonObject.has(JsonConstants.AMOUNT)) {
            ...
            if (!JSONUtils.isNumber(count)) {
                throw new JsonSyntaxException("Amount must be a number larger than 0.");
            }
            amount = count.getAsJsonPrimitive().getAsInt();
            if (amount < 1) {
                throw new JsonSyntaxException("Amount must be larger than or equal to one");
            }
        }
        JsonElement jsonelement =
            JSONUtils.isJsonArray(jsonObject, JsonConstants.INGREDIENT) ? JSONUtils
                .getJsonArray(jsonObject, JsonConstants.INGREDIENT)
                : JSONUtils.getJsonObject(jsonObject, JsonConstants.INGREDIENT);
        Ingredient ingredient = Ingredient.deserialize(jsonelement);
        return create(ingredient, amount);
    }

    public static ItemStackIngredient create(@Nonnull Ingredient ingredient, int amount) {
