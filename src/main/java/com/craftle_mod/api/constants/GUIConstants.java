package com.craftle_mod.api.constants;

import com.craftle_mod.common.Craftle;
import net.minecraft.util.ResourceLocation;

public class GUIConstants {

    public static final int TEXTURE_SIZE = 16;

    public static final int INFO_SCREEN_OFFSET_X = 7;
    public static final int INFO_SCREEN_OFFSET_Y = 11;
    public static final int INFO_SCREEN_WIDTH = 120;
    public static final int INFO_SCREEN_HEIGHT = 64;

    public static final int ENERGY_BAR_X = 159;
    public static final int ENERGY_BAR_Y = 11;
    public static final int ENERGY_BAR_WIDTH = 6;
    public static final int ENERGY_BAR_HEIGHT = 64;

    public static final int BURN_BAR_X = 136;
    public static final int BURN_BAR_Y = 40;
    public static final int BURN_BAR_WIDTH = 14;
    public static final int BURN_BAR_HEIGHT = 14;

    // CRUSHER
    public static final ResourceLocation CRUSHER_BASIC = new ResourceLocation(Craftle.MODID,
        "textures/gui/crusher_basic.png");
    public static final ResourceLocation CRUSHER_TIER_1 = new ResourceLocation(Craftle.MODID,
        "textures/gui/crusher_tier_1.png");
    public static final ResourceLocation CRUSHER_TIER_2 = new ResourceLocation(Craftle.MODID,
        "textures/gui/crusher_tier_2.png");
    public static final ResourceLocation CRUSHER_TIER_3 = new ResourceLocation(Craftle.MODID,
        "textures/gui/crusher_tier_3.png");
    public static final ResourceLocation CRUSHER_TIER_4 = new ResourceLocation(Craftle.MODID,
        "textures/gui/crusher_tier_4.png");

    // GENERATORS
    public static final ResourceLocation COAL_GENERATOR = new ResourceLocation(Craftle.MODID,
        "textures/gui/coal_generator.png");

    // GENERATORS
    public static final ResourceLocation WORKBENCH = new ResourceLocation(Craftle.MODID,
        "textures/gui/workbench.png");

    // ENERGY MATRIX
    public static final ResourceLocation ENERGY_MATRIX = new ResourceLocation(Craftle.MODID,
        "textures/gui/energy_matrix.png");

    public static final ResourceLocation SLOT_OVERLAY_INJECT = new ResourceLocation(Craftle.MODID,
        "textures/gui/overlay/inject.png");
    public static final ResourceLocation SLOT_OVERLAY_EXTRACT = new ResourceLocation(Craftle.MODID,
        "textures/gui/overlay/extract.png");
    public static final ResourceLocation SLOT_OVERLAY_ENERGY = new ResourceLocation(Craftle.MODID,
        "textures/gui/overlay/energy.png");

    public static final ResourceLocation BASE = new ResourceLocation(Craftle.MODID,
        "textures/gui/base.png");
    public static final ResourceLocation BASE_ALT = new ResourceLocation(Craftle.MODID,
        "textures/gui/base_alt.png");

    public static final ResourceLocation ENERGY_BAR_VERTICAL = new ResourceLocation(Craftle.MODID,
        "textures/gui/bar/vertical_energy.png");
    public static final ResourceLocation ENERGY_BAR_HORIZONTAL = new ResourceLocation(Craftle.MODID,
        "textures/gui/bar/horizontal_energy.png");
    public static final ResourceLocation GENERAL_BAR = new ResourceLocation(Craftle.MODID,
        "textures/gui/bar/general.png");
    public static final ResourceLocation BURN_BAR = new ResourceLocation(Craftle.MODID,
        "textures/gui/bar/burn.png");

    public static final ResourceLocation SLOT_DEFAULT = new ResourceLocation(Craftle.MODID,
        "textures/gui/slot/default.png");
    public static final ResourceLocation SLOT_ENERGY = new ResourceLocation(Craftle.MODID,
        "textures/gui/slot/energy.png");
    public static final ResourceLocation SLOT_INJECT = new ResourceLocation(Craftle.MODID,
        "textures/gui/slot/inject.png");
    public static final ResourceLocation SLOT_EXTRACT = new ResourceLocation(Craftle.MODID,
        "textures/gui/slot/extract.png");

}
